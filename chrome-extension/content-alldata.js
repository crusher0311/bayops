// BayOPS Labor Guide Connector - AllData Content Script
// Injects VIN, captures labor times, and syncs to BayOPS

console.log('BayOPS Labor Guide: AllData content script loaded');
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('AllData content script received:', message.type);
...
// Attempt to fill VIN in AllData's vehicle selection
function attemptVinFill() {
  // AllData typically has VIN input in their vehicle selector
  const vinInputs = document.querySelectorAll(
    'input[name*="vin" i], input[placeholder*="vin" i], input[id*="vin" i], ' +
    'input[aria-label*="vin" i], #vin, .vin-input, [data-testid*="vin"]'
  );
  ...
  for (const input of vinInputs) {
    if (input && vinToFill) {
      ...
      console.log('BayOPS: Filled VIN in AllData:', vinToFill);
      return true;
    }
  }
  ...
  button.innerHTML = `
    ...
    ">
      <div style="
        ...
        max-width: 280px;
      ">
        <div style="font-weight: 600; margin-bottom: 4px;">BayOPS Connected - AllData</div>
...
// Capture labor times from AllData page
function captureLaborTimes() {
  ...
  // AllData labor patterns - look for labor time tables
  const laborRows = document.querySelectorAll(
    '.labor-row, .operation-item, tr.labor, [class*="labor-time"], ' +
    '.repair-operation, .service-item, [data-labor]'
  );
  
  laborRows.forEach(row => {
    const description = extractText(row, '.description, .operation, .title, td:first-child, .name');
    const hours = extractNumber(row, '.time, .hours, .labor-hours, [class*="time"]');
    const operationCode = extractText(row, '.code, .op-code, [class*="code"]');
